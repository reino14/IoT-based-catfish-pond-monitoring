...
const findRefBySize = (refs, size) =>
  (refs || []).find((r) => norm(r.ukuran) === norm(size));
...
export default function Ikan() {
  ...
  const addLog = (action, oldData = null, newData = null, quantityChange = null) => {
    ...
    if (action === "Create" && newData) {
      ...
      detail = `${newData.nama_ikan || newData.species || "-"} ukuran ${
        newData.size || newData.ukuran || "-"
      } berhasil ditambahkan. Jumlah: ${newData.quantity || 0} ekor, Berat total: ${
        newData.total_kg ?? "-"
      } kg, Harga: Rp ${
      ...
      const changes = [];
      if (oldData.nama_ikan !== newData.nama_ikan)
        changes.push(`Nama: ${oldData.nama_ikan || "-"} → ${newData.nama_ikan || "-"}`);
      if ((oldData.size || oldData.ukuran) !== (newData.size || newData.ukuran))
        changes.push(`Ukuran: ${oldData.size || oldData.ukuran || "-"} → ${newData.size || newData.ukuran || "-"}`);
      if (oldData.quantity !== newData.quantity)
        changes.push(`Jumlah: ${oldData.quantity || 0} → ${newData.quantity || 0}`);
      ...
      detail = `${nama} (id:${newData.id}) diperbarui: ${
        changes.length ? changes.join(", ") : "tidak ada perubahan field penting"
      }`;
    } else if ((action === "AddStock" || action === "ReduceStock") && oldData && quantityChange !== null) {
      const sign = action === "AddStock" ? "+" : "-";
      detail = `${oldData.nama_ikan || oldData.species || "Unknown"} (id:${oldData.id}) stok ${sign}${Math.abs(
        quantityChange
      )}. Jumlah: ${oldData.quantity} → ${oldData.quantity + quantityChange}`;
    } else if (action === "Delete" && oldData) {
      detail = `${oldData.nama_ikan || oldData.species || "-"} ukuran ${
        oldData.ukuran || oldData.size || "-"
      } berhasil dihapus.`;
    }
  ...
  const filterSizeOptions = useMemo(
    () => uniq((refUkuran || []).map((r) => r.ukuran)),
    [refUkuran]
  );
  const formSizeOptions = filterSizeOptions;

  // ref ukuran terpilih: berdasarkan UKURAN saja
  const selectedRefUkuran = useMemo(
    () => findRefBySize(refUkuran, formData.ukuran),
    [refUkuran, formData.ukuran]
  );
  ...
  const finalSpeciesPreview = useMemo(
    () => buildSpeciesName(selectedRefUkuran?.name, formData.nama_ikan),
    [selectedRefUkuran?.name, formData.nama_ikan]
  );
  ...
  const totalAsset = useMemo(
    () => ikanData.reduce((sum, ikan) => sum + computeAssetValue(ikan, refUkuran), 0),
    [ikanData, refUkuran]
  );
  ...
  const openEditForm = (ikan) => {
    ...
    const perKg =
      totalKgFromIkan && Number(totalKgFromIkan) > 0
        ? Math.round(qty / parseFloat(totalKgFromIkan))
        : "";
    ...
    try {
      ...
      if (formMode === "add") {
        ...
        if (!formData.ukuran || !qpk || !tkg || computedQty <= 0) {
          setErrorMessage(
            "Isi: Ukuran, jumlah ekor/kg, jumlah kg (total) — sehingga jumlah ekor > 0."
          );
          return;
        }
        ...
        try {
          ...
          console.warn(
            "Gagal catat transaksi (pengeluaran) untuk penambahan ikan:",
            financeErr?.response?.data || financeErr.message || financeErr
          );
        }
        ...
        try {
          ...
          console.warn(
            "Gagal catat transaksi (penyesuaian edit):",
            financeErr?.response?.data || financeErr.message || financeErr
          );
        }
        ...
        if (formData.quantity_per_kg && formData.total_kg) {
          change = Math.round(
            (safeParseFloat(formData.quantity_per_kg) || 0) *
              (safeParseFloat(formData.total_kg) || 0)
          );
        } else {
          change = safeParseInt(
            formData.computed_quantity || formData.quantity_per_kg || 0
          );
        }
        ...
        const tipe =
          oldData.price_per_unit != null
            ? "ukuran"
            : oldData.price_per_kg != null
            ? "berat"
            : computePricingType(oldData, refUkuran);
        ...
        setIkanData((prev) =>
          prev.map((i) =>
            i.id === selectedIkanId ? { ...i, quantity: resQty.data.quantity } : i
          )
        );
        addLog(
          formMode === "add_stock" ? "AddStock" : "ReduceStock",
          oldData,
          { ...oldData, quantity: resQty.data.quantity },
          change
        );
        ...
        if (tipe === "berat" && usedKg > 0) {
          ...
          try {
            ...
            setIkanData((prev) =>
              prev.map((i) =>
                i.id === selectedIkanId
                  ? { ...i, total_kg: resKg.data.total_kg ?? resKg.data.avg_weight ?? null }
                  : i
              )
            );
          } catch (e) {
  ...
  const handleDeleteIkan = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Gagal catat transaksi (write-off delete):",
          financeErr?.response?.data || financeErr.message || financeErr
        );
      }
  ...
  const filteredData = ikanData
    ...
    .sort((a, b) => {
      ...
      if (sortField === "harga_per_kg")
        return sortOrder === "asc"
          ? (a.price_per_kg || a.price || 0) - (b.price_per_kg || b.price || 0)
          : (b.price_per_kg || b.price || 0) - (a.price_per_kg || a.price || 0);
      if (sortField === "avg_weight") // map ke total_kg
        return sortOrder === "asc"
          ? (a.total_kg || 0) - (b.total_kg || 0)
          : (b.total_kg || 0) - (a.total_kg || 0);
      return 0;
    });
  ...
  const getPriceInfo = (ikan) => {
    ...
    const unit =
      ikan.price_per_unit != null ? "/ekor" : ikan.price_per_kg != null ? "/kg" : "";
  ...
  return (
    <Layout>
      ...
      <Box mb={3}>
        ...
        >
          <Box>
            <Typography variant="h5" fontWeight="bold">Master Data Ikan 🐟</Typography>
      ...
      <Grid container spacing={3} mb={3}>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            ...
            label="Total Aset"
            value={formatRp(totalAsset)}
            hint="Nilai stok gudang (berdasarkan tipe harga)"
            accent="#6c63ff"
          />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            icon={<Inventory2Icon />}
            label="Total Jenis"
            value={ikanData.length}
            hint="Jumlah item ikan unik"
            accent="#00c9a7"
          />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            ...
            value={(totalQty || 0).toLocaleString("id-ID")}
            accent="#ff7a59"
          />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            ...
            value={(Math.round(totalKg * 100) / 100).toLocaleString("id-ID")}
            accent="#5856d6"
          />
        </Grid>
      </Grid>
      ...
      <Grid container spacing={2}>
        {filteredData.map((ikan) => {
          ...
          const tipeChip =
            mode === "ukuran" ? "Harga/Ekor" : mode === "berat" ? "Harga/Kg" : "Harga";
                ...
                <Stack spacing={1}>
                  ...
                  <Stack direction="row" spacing={1} mt={1}>
                    ...
                    <Button
                      ...
                      color="error"
                      size="small"
                      onClick={() => openReduceStockForm(ikan)}
                    >
                      − Stok
                    </Button>
                  </Stack>
        ...
        <DialogContent>
          <Stack spacing={2} mt={1}>
            {(formMode === "add" || formMode === "edit") && (
              <>
                <TextField
                  label="Nama Ikan (opsional)"
                  placeholder="cth: Lele Jumbo / Nila Merah (hanya informasi tambahan)"
                  value={formData.nama_ikan}
                  onChange={(e) => setFormData({ ...formData, nama_ikan: e.target.value })}
                  fullWidth
                />
                ...
                <TextField
                  label="Jumlah kg (total berat, kg)"
                  type="number"
                  value={formData.total_kg}
                  onChange={(e) => setFormData({ ...formData, total_kg: e.target.value })}
                  helperText="Total berat batch (dalam kg)"
                  fullWidth
                />

                <TextField
                  label="Jumlah ekor (terhitung)"
                  value={formData.computed_quantity ?? 0}
                  InputProps={{ readOnly: true }}
                  helperText="Jumlah ekor dihitung otomatis: ekor/kg × kg"
                  fullWidth
                />
                ...
                <TextField
                  label={`Harga ${
                    tipeHarga === "berat" ? "(per kg)" : tipeHarga === "ukuran" ? "(per ekor)" : ""
                  }`}
                  type="number"
                  ...
                  helperText={
                    tipeHarga === "berat"
                      ...
                      : tipeHarga === "ukuran"
                      ? "Masukkan harga per ekor"
                      : "Pilih ukuran dulu agar tipe harga diketahui"
                  }
                  fullWidth
                />
            ...
            {(formMode === "add_stock" || formMode === "reduce_stock") && (
              <>
                <Typography variant="body2">
                  Masukkan penambahan/pengurangan stok. Kamu bisa pakai "Jumlah ekor/kg" + "Jumlah kg" atau masukkan langsung jumlah ekor pada field "Jumlah ekor (terhitung)".
                </Typography>
                <TextField
                  label="Jumlah ekor/kg (opsional)"
                  type="number"
                  value={formData.quantity_per_kg}
                  onChange={(e) => setFormData({ ...formData, quantity_per_kg: e.target.value })}
                  fullWidth
      ...
      <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)} fullWidth maxWidth="xs">
        ...
        <DialogContent>
          <Typography>
            Apakah kamu yakin ingin menghapus ikan "{confirmIkanName || "-"}"?
          </Typography>
        </DialogContent>
        <DialogActions>
