...
const formatRp = (v) =>
  typeof v === "number" ? "Rp " + Math.round(v).toLocaleString("id-ID") : "-";
...
export default function Finance() {
  ...
  const refreshAll = async () => {
    ...
    try {
      ...
      if (kolamRes.status === "fulfilled")
        setKolamList(kolamRes.value.data || []);
      ...
      if (feedingRes.status === "fulfilled") {
        ...
        const kolamArr =
          kolamRes.status === "fulfilled" ? kolamRes.value.data || [] : [];
      ...
      // ==== Aset Pakan (gudang)
      const pakanArr = (feedRes.status === "fulfilled"
        ? feedRes.value.data || []
        : []
      ).map((f) => {
      ...
      const globalFeedSummary = feedingData.reduce(
        (acc, log) => {
          const qty = safeNum(
            log.jumlah_kg ?? log.jumlah ?? log.quantity_kg ?? 0
          );
          const stok = log.stok_pakan ?? null;
          const pricePerKg = safeNum(
            stok?.price_per_kg ?? stok?.price ?? log.price_per_kg ?? 0
          );
      ...
      const refs =
        refRes.status === "fulfilled" ? refRes.value.data || [] : [];
      ...
      ikans.forEach((it) => {
        ...
        const tipe =
          refItem.tipe_harga ||
          (it.price_per_kg ? "berat" : it.price_per_unit ? "ukuran" : "berat");
        ...
        if (!speciesMap[key].harga_per_kg && it.price_per_kg)
          speciesMap[key].harga_per_kg = it.price_per_kg;
      });
      ...
      const pengeluaranAll = trans.filter(
        (t) =>
          ...
          String(t.tipe ?? t.kategori ?? "").toLowerCase() === "debit"
      );
      const pengeluaranIkanArr = pengeluaranAll.filter((t) =>
        /ikan|lele|nila|patin|bandeng/i.test(
          String(t.keterangan ?? t.deskripsi ?? t.description ?? "")
        )
      );
      const pengeluaranPakanArr = pengeluaranAll.filter((t) =>
        /pakan|pelet|vitamin/i.test(
          String(t.keterangan ?? t.deskripsi ?? t.description ?? "")
        )
      );
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.json_to_sheet(txData),
      "Transaksi"
    );
    ...
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.json_to_sheet(kolamData),
      "Aset_Kolam"
    );
    ...
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.json_to_sheet(ikanGudang),
      "Aset_Ikan_Gudang"
    );
    ...
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.json_to_sheet(pakanData),
      "Aset_Pakan_Gudang"
    );
  ...
  // UI slices
  const txToShow = showAllTransactions
    ? transactions
        ...
        <Stack spacing={1}>
          {feedingToShow.length === 0 ? (
            ...
            feedingToShow.map((lp, idx) => {
              const qty = safeNum(
                lp.jumlah_kg ?? lp.jumlah ?? lp.quantity_kg ?? 0
              );
              const stok = lp.stok_pakan ?? null;
              const price = safeNum(
                stok?.price_per_kg ?? stok?.price ?? lp.price_per_kg ?? 0
              );
              const kolamName =
                lp.__kolam_name ??
                lp.kolam_name ??
              ...
              return (
                ...
                >
                  <Typography variant="subtitle2" fontWeight={700}>
                    {new Date(
                      lp.created_at ?? lp.tanggal ?? Date.now()
                    ).toLocaleDateString()}
                  </Typography>
            ...
            {items.length === 0 ? (
              ...
              items.map((pi, idx) => (
                <TableRow key={idx} hover>
                  ...
                  <TableCell>
                    {pi.deskripsi ?? pi.description ?? pi.keterangan ?? "-"}
                  </TableCell>
                  <TableCell align="right">
                    {formatRp(Number(pi.jumlah ?? pi.amount ?? pi.jumlah_kg ?? 0))}
                  </TableCell>
                </TableRow>
              ))
            )}
        ...
        >
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Ringkasan aset (berdasarkan snapshot kolam), kas, pengeluaran, dan
        ...
        <>
          ...
          <Grid container spacing={3} mb={3}>
            <Grid item xs={12} sm={6} md={3}>
              <StatCard
                icon={<Inventory2Icon />}
                label="Total Aset Kolam"
                value={formatRp(totalAsetKolam)}
                hint="Aset ikan + biaya pakan & vitamin (akumulasi)"
                accent="#5856d6"
              />
            </Grid>
            <Grid item xs={12} sm={6} md={3}>
            ...
            <Grid item xs={12} sm={6} md={3}>
              <StatCard
                icon={<StoreIcon color="error" />}
                label="Total Pengeluaran"
                value={formatRp(summary.total_pengeluaran)}
                accent="#ff7a59"
              />
            </Grid>
            <Grid item xs={12} sm={6} md={3}>
              <StatCard
                ...
                value={formatRp(summary.total_pemasukan)}
                accent="#6c63ff"
              />
            </Grid>
          </Grid>
          ...
          >
            ...
            >
              ...
              <Chip
                label={`Total: ${formatRp(totalAsetKolam)}`}
                variant="outlined"
                sx={{ borderColor: "#5856d6", color: "#5856d6" }}
              />
            </Stack>
            ...
            <Grid container spacing={2}>
              {kolamToShow.length === 0 ? (
                <Grid item xs={12}>
                  <Typography color="text.secondary">
                    Tidak ada data kolam atau aset.
                  </Typography>
                </Grid>
              ) : (
                kolamToShow.map((k) => (
            ...
            {asetPerKolam.length > 6 && (
              <Box textAlign="center" mt={2}>
                <Button onClick={() => setShowAllKolam(!showAllKolam)}>
                  {showAllKolam
                    ? "Tampilkan Sedikit"
                    : `Lihat Semua (${asetPerKolam.length})`}
                </Button>
              </Box>
            )}
          </Paper>
          ...
          >
            ...
            <Box mt={2}>
              <Typography variant="subtitle2">
                Ringkasan Pemakaian Pakan & Vitamin (dari log pemberian)
              </Typography>
              <Typography>
                Pakan: {(feedSummary.feedKg || 0).toFixed(2)} kg —{" "}
                {formatRp(feedSummary.feedCost || 0)}
              </Typography>
              <Typography>
                Vitamin: {(feedSummary.vitaminKg || 0).toFixed(2)} kg —{" "}
                {formatRp(feedSummary.vitaminCost || 0)}
              </Typography>
            </Box>
          </Paper>
          ...
          >
            ...
            {feedingLogs.length > 5 && (
              <Box textAlign="center" mt={1}>
                <Button onClick={() => setShowAllFeeding(!showAllFeeding)}>
                  {showAllFeeding
                    ? "Tampilkan Sedikit"
